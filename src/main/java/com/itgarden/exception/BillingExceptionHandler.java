...
public class BillingExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<Object> resourceNotFound(ResourceNotFoundException ex, WebRequest request)
			throws Exception {
		ErrorMessage errorMessage = new ErrorMessage(ex.getMessage(), Utils.currentDateTime());
	...
	public final ResponseEntity<Object> dataIntegrity(DataIntegrityViolationException ex, WebRequest request)
			throws Exception {
	...
	public final ResponseEntity<Object> duplicateKey(DuplicateKeyFoundException ex, WebRequest request)
			throws Exception {
		ErrorMessage errorMessage = new ErrorMessage(ex.getMessage(), Utils.currentDateTime());
		return new ResponseEntity<>(errorMessage, HttpStatus.CONFLICT);
	...
	/*
	 * @ExceptionHandler(ValidationException.class) public final
	 * ResponseEntity<Object> validationException(ValidationException ex, WebRequest
	 * request) throws Exception { BindingResult bindingResult =
	 * ex.getBindingResult(); List<String> validationMessages = new ArrayList<>();
	 * List<ObjectError> objectErrors = bindingResult.getAllErrors();
	 * bindingResult.getFieldErrors(); List<FieldError> fr =
	 * bindingResult.getFieldErrors(); for (FieldError f : fr) {
	 * System.out.println(f.getField()); System.out.println(f.getRejectedValue()); }
	 * for (ObjectError objectError : objectErrors) { objectError.getObjectName();
	 * String defaultMessage = objectError.getDefaultMessage();
	 * validationMessages.add(defaultMessage + " Test"); } return new
	 * ResponseEntity<>(validationMessages, HttpStatus.BAD_REQUEST);
	 * 
	 * }
	 */
