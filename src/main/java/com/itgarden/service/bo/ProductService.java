		...
		if (StringUtils.isEmpty(product.getProductCode())) {
			...
			if (purchaseOrder == null) {
				throw new ResourceNotFoundException(
						String.format("There is no Purchase Order in Ready state for Vendor %d and Product Name %s ",
				...
				for (Offer offerId : offerRequest) {
					...
					if (!offer.getStatus().equalsIgnoreCase(STATUS.ACTIVE.name())) {
						throw new InvalidInputException(
								String.format("The offer you are associating is not valid: %s", offer.getOfferName()));
					}
					offers.add(offer);
				}
			}
...
//                productStockCount = vendor.get
		...
		if (newProduct.getStockCount() > 0) {
			...
			responseMessage = ResponseMessage.withResponseData(newProduct, "Product saved Successfully",
					Constants.INFO_TYPE);
		}
...
//        if(offerInfos)
		List<Long> offerIds = productInfo.getOffers().stream().map(OfferInfo::getId).collect(Collectors.toList());
		List<Offer> offers = offerRepository.findOfferByIdIn(offerIds);
		product.setOffers(offers);
		PurchaseOrder purchaseOrder = purchaseOrderRepository
				.findPurchaseOrderByVendorAndProductNameAndPurchaseOrderStatus(product.getVendors().get(0),
						product.getName(), PurchaseOrderStatus.READY);
		...
		responseMessage = ResponseMessage.withResponseData(newProductInfo, "Product saved Successfully",
				Constants.INFO_TYPE);
		return responseMessage;
	}
	...
	public ResponseMessage findResourceById(String id) throws Exception {
		...
		ResponseMessage responseMessage = ResponseMessage.withResponseData(productInfo, Constants.SUCCESS_STATUS,
				Constants.INFO_TYPE);
		return responseMessage;
	}
	...
	public ResponseMessage findAll() throws Exception {
		...
		ResponseMessage responseMessage = ResponseMessage.withResponseData(productInfos, Constants.SUCCESS_STATUS,
				Constants.INFO_TYPE);
		return responseMessage;
	}
}
